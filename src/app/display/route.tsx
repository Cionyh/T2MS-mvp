...
export async function GET(req: NextRequest) {
  try {
    ...
    // Support both 'channel' and 'clientId' for flexibility
    const clientId = searchParams.get("channel") || searchParams.get("clientId");
...
<body>
  <div class="error">
    <h2>Widget Error</h2>
    <p>Missing channel or clientId parameter. Please provide ?channel=YOUR_CLIENT_ID</p>
  </div>
</body>
</html>`,
        {
    ...
    // Fetch client and message data (same logic as API endpoint)
...
<body>
  <div class="error">
    <h2>Widget Error</h2>
    <p>Client not found. Please check your channel ID.</p>
  </div>
</body>
</html>`,
        {
    ...
    console.error("❌ Error in display endpoint:", err);
    return new NextResponse(
      `<!DOCTYPE html>
<html>
<head>
...
function generateWidgetHTML(clientId: string, messageData: any, apiBase: string): string {
  ...
  console.log("🔍 Widget type for iframe:", type);
  ...
  function getWidgetHTML() {
    ...
    // Add company link if provided (except for ticker, banner, popup which handle it separately)
    ...
    // For banner and ticker, add link below
