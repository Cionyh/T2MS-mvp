...
const navItems = [
  ...
  { label: "Payouts", href: "/worker/payouts", icon: DollarSign },
];
...
export function WorkerLayoutShell({ children }: { children: ReactNode }) {
    ...
    <div className="flex flex-col min-h-screen bg-muted/40">
      <header className="sticky top-0 z-40 w-full border-b bg-background px-4 py-3 flex justify-between items-center">
        <div className="flex items-center gap-6">
          <Link href="/worker/dashboard" className="text-lg font-semibold text-primary">
            T2MS Worker
          </Link>
            ...
            {navItems.map(({ label, href, icon: Icon }) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === href || (href !== "/worker/dashboard" && pathname.startsWith(href))
                    ? "text-primary"
